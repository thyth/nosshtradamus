...
GO_MOD_CACHE=$(go env GOMODCACHE) || { echo "Couldn't look up GOMODCACHE"; exit 1; }

# Resolve the version of the go-mosh module we want
GO_MOSH_REPO="gitlab.hive.thyth.com/chronostruct/go-mosh"
GO_MOSH_VERSION=$(grep -oP "${GO_MOSH_REPO} \Kv([^ ]*)" "${PROJECT_DIR}/go.mod") || { echo "No go-mosh in go.mod"; exit 1; }
